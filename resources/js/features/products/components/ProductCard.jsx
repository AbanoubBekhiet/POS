...
export default function ProductCard({ product, onAddToCart }) {
    return (
        <div
            className="rounded-2xl overflow-hidden group transition-all duration-300 hover:shadow-lg"
        ...
        >
            ...
            {/* Info */}
            <div className="p-4">
                <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#9A978F' }}>{product.category}</p>
                <h3 className="text-sm font-bold mt-1 line-clamp-1 transition-colors" style={{ color: '#1A2D23' }}>
                    {product.name}
                </h3>
                <div className="flex items-center justify-between mt-3">
                    <p className="text-lg font-bold" style={{ color: '#1A2D23' }}>${product.price.toFixed(2)}</p>
                    <p className="text-xs" style={{ color: '#B8B5AE' }}>{product.stock} in stock</p>
                </div>
            </div>
        </div>
    )
